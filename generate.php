...
function sparklineOld(array $values, string $title = ''): string {
	$values = array_map('floatval', $values);
	$count = count($values);
	if ($count === 0) return '<div class="spark empty">No data</div>';
...
try {
	...
	$activeUsersRow = fetchOneAny($pdo ?? $mysqli, '
		SELECT COUNT(*) AS cnt
		FROM view_redcap_metrics_users
		WHERE COALESCE(last_activity_at, last_login_at, first_activity_at) >= DATE_SUB(NOW(), INTERVAL 30 DAY)
	');

	$activeProjectsRow = fetchOneAny($pdo ?? $mysqli, '
		SELECT COUNT(*) AS cnt
		FROM view_redcap_metrics_projects
		WHERE COALESCE(last_logged_event_at, last_updated, moved_to_production_at, completed_at) >= DATE_SUB(NOW(), INTERVAL 30 DAY)
	');

	$newUsers30d = fetchOneAny($pdo ?? $mysqli, '
		SELECT COUNT(*) AS cnt
		FROM view_redcap_metrics_users
		WHERE account_created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
	 ');
...
<body>
	...
	<div class="hero">
		...
		<div class="hero-inner">
			...
			<div class="meta">Updated <?= h($generatedAt ?? null) ?></div>
		...
		<div class="metric-band">
			<div class="mini"><div class="label">Total users</div><div class="value mono"><?= nfmt($totalUsers) ?></div></div>
			<div class="mini"><div class="label">Total projects</div><div class="value mono"><?= nfmt($totalProjects) ?></div></div>
			<div class="mini"><div class="label">Production projects</div><div class="value mono"><?= nfmt($productionProjects) ?></div><div class="note"><?= pct($productionProjects, $totalProjects) ?> of all projects</div></div>
			<div class="mini"><div class="label">Total records</div><div class="value mono"><?= nfmt($totalRecords) ?></div><div class="note">Records stored across all projects</div></div>
		</div>
		</div>
	</div>
	...
	<div id="overview" class="tab-panel active">
		...
		<div class="card"><div class="k">Active users (30d)</div><div class="v mono"><?= nfmt($activeUsers) ?></div><div class="s">Users active in the last 30 days</div></div>
		<div class="card"><div class="k">Active projects (30d)</div><div class="v mono"><?= nfmt($activeProjects) ?></div><div class="s">Projects with recent activity</div></div>
		<div class="card"><div class="k">Projects with surveys</div><div class="v mono"><?= nfmt($surveys) ?></div><div class="s">Survey-enabled projects</div></div>
		<div class="card"><div class="k">API projects</div><div class="v mono"><?= nfmt($apiProjects) ?></div><div class="s">Projects with API access</div></div>
		</div>

		<div class="grid section-grid">
		<div class="card chart-box">
			<div class="section-title"><h2>Daily new projects</h2><div class="small">Last 30 days</div></div>
			<div class="chart-wrap"><?= sparkline($projectDailyValues, 'Daily new projects over the last 30 days') ?></div>
			<div class="legend">
		...
		<div class="stack">
			<div class="card">
			<div class="section-title"><h2>Project status</h2><div class="small">Distribution by status</div></div>
			<div class="bars">
				...
				<div class="bar-row"><div class="bar-label"><?= h(ucwords($row['label'])) ?></div><div class="bar-track"><div class="bar-fill" style="width: <?= number_format($pctWidth, 2) ?>%"></div></div><div class="bar-val mono"><?= nfmt($row['cnt']) ?></div></div>
				<?php endforeach; ?>
			</div>
			</div>

			<div class="card">
			<div class="section-title"><h2>Project purpose</h2><div class="small">Distribution by purpose</div></div>
			<div class="bars">
	...
	<div id="trends" class="tab-panel">
		...
		<div class="card trends-card table-card">
			<div class="section-title"><h2>User growth</h2><div class="small">New and total users over time</div></div>
		...
		<div class="card trends-card table-card">
			<div class="section-title"><h2>Project growth</h2><div class="small">New and total projects over time</div></div>
			<div class="scroll-table">
			<table>
	...
	<div id="details" class="tab-panel">
		...
		<div class="section-title"><h2>Recently updated projects</h2><div class="small">Most recently updated or created</div></div>
		<div class="scroll-table" style="max-height: 520px;">
			<table>
			<thead><tr><th>Project</th><th>Status</th><th>Purpose</th><th class="mono">Records</th><th class="mono">Users</th><th>Updated</th></tr></thead>
			<tbody>
